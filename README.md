...
| KV PREDICTION FOR IMPROVED TIME TO FIRST TOKEN | It's one of the interesting works from Apple recently. It introduces small model prediction of KV-Cache approximations. Parallels drawn to previously proposed MoE expert activation prediction. Potential implications for utilizing larger total parameters with limited memory. KVP-C and KVP-LP implementations suggest robust activation patterns across different model sizes and pruning scenarios. |
